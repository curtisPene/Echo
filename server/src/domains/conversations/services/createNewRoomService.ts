...
} from "../../authAndAccess/composition";
...
export const createNewRoomService = async ({
  ...
  try {
    ...
    const existenceChecks = await Promise.all(
      participantIds.map((id) => verifyUserIdService.execute({ userId: id })),
    );
    ...
    const creatorContacts = await getUsersContactsService.execute({ userId: user });

    const participantContactsResults = await Promise.all(
      participantIds.map((id) => getUsersContactsService.execute({ userId: id })),
    );
