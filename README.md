...
**The client layers MVVM + DDD on top of that same hexagonal core.** Code is organized by domain (`domains/identity`, `domains/conversation`, `domains/messaging`, …) rather than by technical layer, and each domain follows Model → Service → ViewModel → View: services are pure functions with no framework or store access; a ViewModel is the one place allowed to read/write React or Zustand state, and it's the only thing a View is allowed to call — never a service, never a store, directly. A View also never touches another domain's internals; it can call another domain's ViewModel or service (the same way an HTTP controller calls a service), but never its repo. Presentation models (e.g. `RoomPresentation`) exist specifically so a fact like "who's the other participant" or "is this room pending" is computed once, by the domain that owns it, instead of being re-derived slightly differently everywhere it's needed.
